 ...
 *
 * Drop into your frontend:  src/pages/admin/AdminArchiveDashboard.jsx
 *
 * Features:
 *  - Browse all archived entities (tournament, team, stream, post, team_finder_post)
 *  - Filter by entity type and date range
 *  - Expand any row to inspect the full archived snapshot (with child JSONB)
 *  - One-click restore for tournaments, teams, streams
 *  - Audit log tab with actor, reason, restore status
 *  - Purge old archives (admin danger zone)
 */
...
export default function AdminArchiveDashboard() {
  ...
  const handlePurge = async () => {
    ...
    try {
      ...
      showToast(`❌ Purge failed: ${e.message}`, false);
    } finally {
      setPurging(false);
    }
  };
          ...
          {childKey.map((key) => {
            ...
            return (
              <div key={key} style={styles.childSection}>
                <div style={styles.childTitle}>
                  {key.replace(/_/g, " ").replace("snapshot", "").trim()}
                  <span style={styles.childCount}>{arr.length}</span>
                </div>
