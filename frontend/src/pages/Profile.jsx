    ...
    const load = async () => {
      try {
        ...
        try {
          ...
          const tr = await fetch("/api/teams/mine", { headers: { Authorization: `Bearer ${token}` } });
          ...
          <div className="rounded-xl border border-surface-border bg-navy/40 p-4 mb-4">
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="w-16 h-16 rounded-full border-2 border-surface-border overflow-hidden shrink-0 bg-red/10 flex items-center justify-center relative">
                {avatarPreview
                  ? <img src={avatarPreview} alt="preview" className="w-full h-full object-cover" onError={(e) => { e.target.style.display = "none"; }} />
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex gap-1 mb-2">
                  <button type="button" onClick={() => { setAvatarUrlMode(false); clearAvatar(); }}
                    className={"px-2.5 py-1 rounded-lg text-xs font-medium transition-colors " + (!avatarUrlMode ? "bg-red/20 text-red border border-red/30" : "text-gray-500 hover:text-white border border-surface-border")}>
                    Upload file
                  </button>
                  <button type="button" onClick={() => { setAvatarUrlMode(true); clearAvatar(); }}
                    className={"px-2.5 py-1 rounded-lg text-xs font-medium transition-colors " + (avatarUrlMode ? "bg-red/20 text-red border border-red/30" : "text-gray-500 hover:text-white border border-surface-border")}>
                    Paste URL
                  </button>
                  {avatarPreview && (
                ...
                {!avatarUrlMode ? (
                  <div>
                    <input
                      ...
                      className="hidden"
                      onChange={(e) => { avatarUpload.pickFile(e.target.files?.[0]); e.target.value = ""; }}
                    />
                    <button
                      type="button"
                ...
                {/* Error */}
                {avatarUpload.error && (
                  <p className="text-xs mt-1.5 flex items-center gap-1" style={{ color: "#ff4655" }}>
                    ⚠ {avatarUpload.error}
                  </p>
                )}

                {/* Success */}
                {avatarPreview && !avatarUpload.processing && !avatarUpload.error && (
                  <p className="text-xs text-green-400 mt-1.5 flex items-center gap-1">
                    <span className="w-1.5 h-1.5 rounded-full bg-green-400 inline-block" /> Ready to save
                  </p>
                )}
              </div>
            </div>
        ...
        <div className="space-y-3 animate-fade-in">
          {myTeams.length === 0 ? (
            <div className="card text-center py-10 text-gray-500">
              ...
              <p className="font-medium text-gray-400">You're not on any teams yet</p>
              <p className="text-sm mt-1">Create a team in <a href="/teamfinder" className="text-red hover:underline">LFG Hub</a> to start recruiting</p>
            </div>
          ) : myTeams.map((team) => (
            <div key={team.team_id} className="card flex flex-wrap items-center gap-4">
              <div className="w-10 h-10 rounded-xl overflow-hidden shrink-0 flex items-center justify-center bg-red/10 border border-red/20">
                {team.game_icon ? <img src={team.game_icon} alt="" className="w-full h-full object-cover" /> : <span className="text-lg">⚔️</span>}
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                  <p className="font-semibold text-white">{team.team_name}</p>
                  {team.game_name && <span className="badge-red text-xs">🎮 {team.game_name}</span>}
                  {team.region && <span className="badge-gray text-xs">📍 {team.region}</span>}
                </div>
                {/* Member avatars */}
                <div className="flex items-center gap-1 mt-1.5 flex-wrap">
                  {(team.members || []).slice(0, 6).map((m) => (
                    <a key={m.user_id} href={`/users/${m.user_id}`} title={m.username} className="relative hover:scale-110 transition-transform">
                      ...
                      {m.role === "captain" && <span className="absolute -top-0.5 -right-0.5 text-[7px] leading-none">⭐</span>}
                    </a>
                  ))}
                  {(team.members || []).length > 6 && <span className="text-xs text-gray-500">+{team.members.length - 6}</span>}
                  <span className="text-xs text-gray-600 ml-1">{(team.members || []).length} members</span>
                </div>
              </div>
              <div className="flex items-center gap-2 shrink-0">
                <span className={`text-xs px-2.5 py-1 rounded-full font-semibold ${team.my_role === "captain" ? "bg-red/15 border border-red/30 text-red" : "bg-white/5 border border-white/10 text-gray-400"}`}>
                  {team.my_role === "captain" ? "⭐ Captain" : "Member"}
                </span>
                <a href="/teamfinder" className="text-xs px-2.5 py-1 rounded-lg border border-surface-border text-gray-500 hover:text-white hover:border-red/30 transition-colors">Manage</a>
              </div>
            </div>
          ))}
        </div>
