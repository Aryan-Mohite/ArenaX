    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backdropFilter: "blur(8px)", background: "rgba(2,6,23,0.75)" }} onClick={onClose}>
      <div className="w-full max-w-sm rounded-2xl border border-surface-border overflow-hidden animate-slide-up" style={{ background: "linear-gradient(145deg,#1a2340,#131a2e)" }} onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-surface-border">
          <div><h3 className="font-display font-bold text-white">Share Profile</h3><p className="text-xs text-gray-500 mt-0.5">@{profile?.username}</p></div>
  ...
  return (
    <div className="rounded-2xl border border-surface-border overflow-hidden" style={{ background: "linear-gradient(145deg,#1a2340,#131a2e)" }}>
      <div className="flex items-center gap-3 px-5 py-4 border-b border-surface-border">
        ...
        <div className="flex-1 min-w-0"><p className="font-semibold text-white text-sm">{game.game_name}</p><p className="text-xs text-gray-500">Auto-fetch enabled</p></div>
        <div className="flex flex-wrap gap-1.5 shrink-0">{game.rank && <span className="badge-blue">{game.rank}</span>}{game.elo_rating && <span className="badge-gray">ELO {game.elo_rating}</span>}</div>
      </div>
      ...
      {result && (
        <div className="mx-5 mb-5 rounded-xl border border-red/20 overflow-hidden" style={{ background: "linear-gradient(135deg,rgba(255,70,85,0.06),rgba(26,35,64,0.8))" }}>
          ...
          <div className="px-4 pb-4 flex items-center justify-between gap-3">
            ...
            {saved ? <span className="badge-green text-xs">✓ Saved!</span> : <button onClick={handleSave} className="btn-primary text-xs px-4 py-2 flex items-center gap-1.5">💾 Save to Profile</button>}
          </div>
        </div>
      )}
    </div>
...
export default function Profile() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setEditMode(false); showToast("Profile updated!");
    } catch (err) { setError(err.response?.data?.message || "Failed to save"); } finally { setSaving(false); }
  };

  const handleGameStatsSave = async (data) => { await upsertGameProfile(data); const res = await getMyGames(); setGames(res.data.games || []); showToast("Game stats saved!"); };
  ...
  const TABS = [{ id: "overview", label: "Overview" }, { id: "games", label: "Games" }, { id: "gamestats", label: "⚡ Live Stats" }];
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-10 animate-fade-in">
      ...
      <div className="card mb-6 relative overflow-hidden">
        ...
        <div className="relative flex flex-col sm:flex-row items-start sm:items-center gap-5">
          ...
          <div className="flex items-center gap-2 self-start shrink-0">
            ...
            <button onClick={() => setEditMode(!editMode)} className="btn-secondary text-sm">{editMode ? "Cancel" : "Edit Profile"}</button>
          </div>
        </div>
      </div>
      ...
      {editMode && (
        <div className="card mb-6 animate-slide-up">
          ...
          <div className="flex justify-end mt-4"><button onClick={handleSave} disabled={saving} className="btn-primary">{saving ? "Saving..." : "Save Changes"}</button></div>
        </div>
      )}
      ...
      {activeTab === "games" && (
        <div className="space-y-3 animate-fade-in">
          {games.length === 0 ? <div className="card text-center py-10 text-gray-500">No games in library. <a href="/games" className="text-red hover:underline">Browse games →</a></div>
            : games.map((game) => (
        ...
        <div className="animate-fade-in">
          <div className="rounded-2xl border border-red/20 mb-6 px-5 py-4 flex gap-4 items-start" style={{ background: "linear-gradient(135deg,rgba(255,70,85,0.08),rgba(26,35,64,0.6))" }}>
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 leading-relaxed">Enter your username for supported games to pull live rank &amp; ELO from official APIs. Hit <strong className="text-white">Save to Profile</strong> to sync.</p>
              <div className="flex flex-wrap gap-2 mt-3">
