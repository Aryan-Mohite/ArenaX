...
const GAMES = [
  ...
  { name: 'Overwatch',     color: '#f99e1a', abbr: 'OW2' },
  { name: 'Rocket League', color: '#0082ff', abbr: 'RL'  },
]
...
function EmptyState({ icon, title, message, linkTo, linkLabel }) {
  return (
    <div className="card text-center py-14 flex flex-col items-center gap-3">
      ...
      <Link to={linkTo} className="btn-ghost text-sm mt-1">{linkLabel} →</Link>
    </div>
  )
}
...
function ErrorState({ message, onRetry }) {
  return (
    <div className="card text-center py-12 border-red/20">
      ...
      <button onClick={onRetry} className="btn-secondary text-sm px-6 py-2">Retry</button>
    </div>
  )
}
    ...
    <div className="animate-fade-in">
      ...
      <section className="relative overflow-hidden min-h-[92vh] flex items-center">
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <div className="max-w-2xl">
              ...
              <div className="flex flex-wrap gap-4 mb-10">
                ...
                <Link to="/tournament" className="btn-primary text-base px-8 py-3 shadow-lg shadow-red/20">
                  Browse Tournaments
                </Link>
                ...
                <div className="flex -space-x-2">
                  {['A','K','R','M','J'].map((l, i) => (
                    <div key={i}
                      ...
                      style={{ background: ['#ff4655','#f4a523','#00d4ff','#c89b3c','#fc4b08'][i] }}
                    >{l}</div>
                  ))}
                </div>
                <p className="text-sm text-gray-400">
                  Join <span className="text-white font-semibold">50,000+</span> players already competing
                </p>
              </div>
            </div>
            ...
            <div className="hidden lg:flex items-center justify-center relative">
              <div className="relative w-full max-w-md aspect-square">
                {/* Glowing ring */}
                <div className="absolute inset-0 rounded-full border border-red/20 animate-ping opacity-20" style={{ animationDuration: '3s' }} />
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="w-40 h-40 rounded-full bg-red/10 border border-red/30 flex items-center justify-center backdrop-blur-sm">
                    <svg width="64" height="64" viewBox="0 0 64 64" fill="none">
                      <path d="M32 8L56 20V44L32 56L8 44V20L32 8Z" stroke="#ff4655" strokeWidth="1.5" fill="none" opacity="0.6"/>
                      <path d="M32 16L48 25V43L32 52L16 43V25L32 16Z" fill="#ff4655" opacity="0.15"/>
                      <circle cx="32" cy="32" r="8" fill="#ff4655" opacity="0.8"/>
                      <path d="M24 32H18M40 32H46M32 24V18M32 40V46" stroke="#ff4655" strokeWidth="2" strokeLinecap="round"/>
                    </svg>
                  </div>
                </div>
              ...
              <div className="absolute inset-0 pointer-events-none"
                style={{ background: 'radial-gradient(ellipse at center, #ff465520 0%, transparent 70%)' }} />
            </div>
          </div>
        </div>
        ...
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-1 opacity-40">
          <div className="w-5 h-8 rounded-full border border-gray-600 flex items-start justify-center pt-1.5">
            <div className="w-1 h-2 rounded-full bg-gray-400" style={{ animation: 'scrollDot 1.8s ease-in-out infinite' }} />
          </div>
        </div>
      </section>
      ...
      <div className="border-y border-surface-border bg-surface-card/30 py-5 overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <p className="text-xs text-gray-600 uppercase tracking-widest mb-4">Supported games</p>
          <div className="flex gap-3 overflow-x-auto pb-1 scrollbar-hide">
            {GAMES.map(g => (
              <div key={g.name}
                ...
                style={{ borderColor: g.color + '40', color: g.color, background: g.color + '12' }}
              >{g.name}</div>
            ))}
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-16 space-y-20">
        ...
        <section>
          ...
          {loadingStreams ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {[1,2,3,4].map(i => <StreamSkeletonCard key={i} />)}
            </div>
          ) : errorStreams ? (
            <ErrorState
                    ...
                    <div className="p-3 flex-1 flex flex-col gap-2">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-gray-500 truncate">{s.game_name}</p>
                        </div>
                      </div>

                      <p className="text-xs text-gray-400 line-clamp-2 leading-relaxed">{s.title}</p>
        ...
        <section className="relative overflow-hidden rounded-2xl border border-surface-border">
          ...
          <div className="absolute inset-0 pointer-events-none"
            style={{ background: 'radial-gradient(ellipse at 30% 50%, #ff465515 0%, transparent 60%)' }} />
          ...
          <div className="relative px-8 md:px-16 py-16 text-center">
              ...
              <div className="flex -space-x-2">
                {['T','S','V','A','N'].map((l, i) => (
                  <div key={i}
                    ...
                    style={{ background: ['#ff4655','#00d4ff','#c89b3c','#fc4b08','#f4a523'][i] }}
                  >{l}</div>
                ))}
              </div>
              <p className="text-sm text-gray-400 ml-1">50,000+ players competing</p>
            </div>
            ...
            <div className="flex justify-center gap-4 flex-wrap">
              <Link to="/register" className="btn-primary px-8 py-3 text-base shadow-lg shadow-red/20">
                Create Free Account
              </Link>
      ...
      {/* ══════════════════════════════════════════════════════
          FOOTER
      ══════════════════════════════════════════════════════ */}
      <footer className="relative border-t border-surface-border bg-surface-card/40 overflow-hidden mt-16">
        {/* Subtle red glow from bottom — matches site palette */}
        <div
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 py-10 flex flex-col items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            {/* Two-tone wordmark — replace with your actual site name */}
