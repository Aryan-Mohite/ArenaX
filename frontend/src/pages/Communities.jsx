...
function Avatar({ user, size = 9 }) {
  ...
  if (user?.profile_picture) return <img src={user.profile_picture} alt={user.username} className={`${s} rounded-full object-cover border border-surface-border shrink-0`} />
          ...
          <div className="flex items-center justify-between mb-5 gap-4 flex-wrap">
            <div>
              {viewMode === 'community' && activeCommunity ? (
                ...
                <>
                  <h2 className="font-display font-bold text-xl text-white">All Posts</h2>
